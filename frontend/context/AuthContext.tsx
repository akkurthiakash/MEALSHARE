...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const login = async (email: string, pass: string): Promise<UserProfile> => {
    ...
    // 1. Check Admin Verification Endpoint on Backend
    try {
    ...
    // 2. Standard User Authentication via Firebase / Auth Engine
    try {
      ...
      if (pass.length >= 6) {
        ...
        const genUser: UserProfile = {
          uid: isSystemAdmin ? 'admin-primary' : 'user-' + Date.now(),
          email: normEmail,
          displayName: isSystemAdmin ? 'System Admin' : normEmail.split('@')[0],
          role: isSystemAdmin ? 'admin' : 'user',
          createdAt: new Date().toISOString().split('T')[0]
        };
        setUser(genUser);
        localStorage.setItem('mealshare_mock_user', JSON.stringify(genUser));
        document.cookie = `token=mock-token-${genUser.uid}; path=/; max-age=86400`;
        document.cookie = `user_role=${genUser.role}; path=/; max-age=86400`;
        return genUser;
      }
  ...
  const loginWithGoogle = async (): Promise<UserProfile | null> => {
    try {
      ...
      const userObj: UserProfile = {
        ...
        displayName: res.user.displayName || res.user.email?.split('@')[0] || 'Chef',
        role: isSystemAdmin ? 'admin' : 'user',
        createdAt: new Date().toISOString().split('T')[0]
      };
      setUser(userObj);
      ...
      document.cookie = `token=mock-token-google; path=/; max-age=86400`;
      document.cookie = `user_role=user; path=/; max-age=86400`;
      return demoUser;
    }
  };
