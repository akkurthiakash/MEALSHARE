...
export default function LoginPage() {
  ...
  const performGoogleAuth = (emailToUse: string, nameToUse: string) => {
    setLoading(true);
    ...
    try {
      ...
      document.cookie = `token=mock-token-active; path=/; max-age=86400`;
      document.cookie = `user_role=user; path=/; max-age=86400`;
      toast.success(`Signed in as ${emailToUse}`);
      window.location.href = '/dashboard';
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-[#FDF7F2] dark:bg-[#0b1324] relative flex flex-col justify-between overflow-x-hidden font-serif text-[#0F172A] dark:text-slate-100 -m-8 transition-colors duration-200">
      ...
      {showGoogleModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 animate-in fade-in zoom-in duration-200 font-sans">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold text-sm text-slate-800 dark:text-slate-200">Choose an account</span>
              </div>
              <button 
            ...
            <p className="text-xs text-slate-500">to continue to <strong className="text-slate-700 dark:text-slate-300">MealShare</strong></p>

            {/* Account Card */}
            <button
              onClick={() => performGoogleAuth('akkurthiakash3015@gmail.com', 'Chef Akash')}
              className="w-full flex items-center space-x-3 p-3 rounded-2xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-left group"
            >
              <div className="w-10 h-10 rounded-full bg-emerald-600 text-white font-extrabold flex items-center justify-center text-sm shadow-sm group-hover:scale-105 transition-transform">
                A
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-sm text-slate-900 dark:text-white truncate">Chef Akash</div>
                <div className="text-xs text-slate-500 truncate">akkurthiakash3015@gmail.com</div>
              </div>
            </button>
